...
BEGIN_MESSAGE_MAP(CButtonStruct, CButton)
    //{{AFX_MSG_MAP(CButtonStruct)
	ON_WM_SETCURSOR()
	ON_WM_KILLFOCUS()
	ON_WM_MOUSEMOVE()
...
void CButtonStruct::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		// Destroy icons
...
void CButtonStruct::PreSubclassWindow() 
{
	UINT nBS;
...
UINT CButtonStruct::OnGetDlgCode() 
{
...
BOOL CButtonStruct::PreTranslateMessage(MSG* pMsg) 
{
	InitToolTip();
...
HBRUSH CButtonStruct::CtlColor(CDC* pDC, UINT nCtlColor) 
{
	return (HBRUSH)::GetStockObject(NULL_BRUSH); 
} // End of CtlColor

void CButtonStruct::OnSysColorChange() 
{
...
LRESULT CButtonStruct::OnSetStyle(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CButtonStruct::OnSetCheck(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CButtonStruct::OnGetCheck(WPARAM wParam, LPARAM lParam)
{
	ASSERT(m_bIsCheckBox);
	return GetCheck();
} // End of OnGetCheck

void CButtonStruct::OnEnable(BOOL bEnable) 
{
...
void CButtonStruct::OnKillFocus(CWnd * pNewWnd)
{
	CButton::OnKillFocus(pNewWnd);
	CancelHover();
} // End of OnKillFocus

void CButtonStruct::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
	CButton::OnActivate(nState, pWndOther, bMinimized);
	if (nState == WA_INACTIVE)	CancelHover();
} // End of OnActivate

void CButtonStruct::OnCancelMode() 
{
	CButton::OnCancelMode();
	CancelHover();
} // End of OnCancelMode

BOOL CButtonStruct::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
void CButtonStruct::CancelHover()
{
...
void CButtonStruct::OnMouseMove(UINT nFlags, CPoint point)
{
...
LRESULT CButtonStruct::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	CancelHover();
	return 0;
} // End of OnMouseLeave

BOOL CButtonStruct::OnClicked() 
{	
	SetFocus();
...
void CButtonStruct::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
...
void CButtonStruct::PaintBk(CDC* pDC)
{
...
HBITMAP CButtonStruct::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor)
{
	HBITMAP		hMask		= NULL;
	HDC			hdcSrc		= NULL;
	HDC			hdcDest		= NULL;
...
//
void CButtonStruct::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage)
{
	CRect rBtn;
...
void CButtonStruct::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	BYTE		byIndex		= 0;
...
void CButtonStruct::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	HDC			hdcBmpMem	= NULL;
...
void CButtonStruct::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
...
//
HICON CButtonStruct::CreateGrayscaleIcon(HICON hIcon)
{
...
//
DWORD CButtonStruct::SetIcon(int nIconIn, int nIconOut)
{
	HICON		hIconIn			= NULL;
	HICON		hIconOut		= NULL;
...
//			Failed loading the specified resource.
//
DWORD CButtonStruct::SetIcon(HICON hIconIn, HICON hIconOut)
{
	BOOL		bRetValue;
	ICONINFO	ii;
...
//
DWORD CButtonStruct::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
...
//			Failed creating mask bitmap.
//
DWORD CButtonStruct::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	int		nRetValue;
	BITMAP	csBitmapSize;
...
//				By default, CButtonStruct buttons are flat.
//		[IN]	bRepaint
...
//			Function executed successfully.
//
DWORD CButtonStruct::SetFlat(BOOL bFlat, BOOL bRepaint)
{
	m_bIsFlat = bFlat;
...
//				By default, CButtonStruct buttons have ST_ALIGN_HORIZ alignment.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CButtonStruct::SetAlign(BYTE byAlign, BOOL bRepaint)
{
	switch (byAlign)
	{    
		case ST_ALIGN_HORIZ:
...
//				By default, CButtonStruct buttons have BTNST_PRESSED_LEFTRIGHT style.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CButtonStruct::SetPressedStyle(BYTE byStyle, BOOL bRepaint)
{
	switch (byStyle)
	{
...
//			Function executed successfully.
//
DWORD CButtonStruct::SetCheck(int nCheck, BOOL bRepaint)
{
	if (m_bIsCheckBox)
	{
...
//			Function executed successfully.
//
DWORD CButtonStruct::SetDefaultColors(BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CButtonStruct::SetColor(BYTE byColorIndex, COLORREF crColor, BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CButtonStruct::GetColor(BYTE byColorIndex, COLORREF* crpColor)
{
	if (byColorIndex >= BTNST_MAX_COLORS)	return BTNST_INVALIDINDEX;
...
//
DWORD CButtonStruct::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint)
{
	BYTE	byRed = 0;
	BYTE	byGreen = 0;
	BYTE	byBlue = 0;
...
//			Function executed successfully.
//
DWORD CButtonStruct::SetAlwaysTrack(BOOL bAlwaysTrack)
{
...
//			Failed loading the specified resource.
//
DWORD CButtonStruct::SetBtnCursor(int nCursorId, BOOL bRepaint)
{
	HINSTANCE	hInstResource = NULL;
...
//			Function executed successfully.
//
DWORD CButtonStruct::DrawBorder(BOOL bDrawBorder, BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CButtonStruct::DrawFlatFocus(BOOL bDrawFlatFocus, BOOL bRepaint)
{
...
void CButtonStruct::InitToolTip()
{
...
//
void CButtonStruct::SetTooltipText(int nText, BOOL bActivate)
{
	CString sText;
...
//				If TRUE the tooltip will be created active.
//
void CButtonStruct::SetTooltipText(LPCTSTR lpszText, BOOL bActivate)
{
...
//
void CButtonStruct::ActivateTooltip(BOOL bActivate)
{
...
//			Function executed successfully.
//
DWORD CButtonStruct::EnableBalloonTooltip()
{
...
//				If TRUE the control will be repainted.
//
void CButtonStruct::DrawTransparent(BOOL bRepaint)
{
...
DWORD CButtonStruct::SetBk(CDC* pDC)
{
...
//			Function executed successfully.
//
DWORD CButtonStruct::SetURL(LPCTSTR lpszURL)
{
...
//
#ifndef	BTNST_USE_BCMENU
DWORD CButtonStruct::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bRepaint)
{
	HINSTANCE	hInstResource	= NULL;
...
//
void CButtonStruct::SizeToContent()
{
	if (m_csIcons[0].hIcon)
	{
		m_ptImageOrg.x = 0;
...
//
#ifdef	BTNST_USE_SOUND
DWORD CButtonStruct::SetSound(LPCTSTR lpszSound, HMODULE hMod, BOOL bPlayOnClick, BOOL bPlayAsync)
{
...
// This is a virtual function that can be rewritten in CButtonStruct-derived classes
...
//			Function executed successfully.
//
DWORD CButtonStruct::OnDrawBackground(CDC* pDC, CRect* pRect)
{
	COLORREF	crColor;
...
//			Function executed successfully.
//
DWORD CButtonStruct::OnDrawBorder(CDC* pDC, CRect* pRect)
{
	// Draw pressed button
	if (m_bIsPressed)
	{
